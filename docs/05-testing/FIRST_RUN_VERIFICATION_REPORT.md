...
|---|---|---|
| `pnpm run verify` | Passed | Production/training boundary, secret scan, 78/25 requirement traceability with 227 evidence paths, generated OpenAPI drift, 12 Bicep templates, four container-definition targets, strict TypeScript, 99 unit/integration/security/acceptance tests, and compiled API/worker/browser process checks |
...
| `pnpm run openapi:verify` | Passed within `verify` | TypeScript-derived runtime schemas match 195 active `/v1` routes; generated OpenAPI 3.0.3 publishes 155 request bodies, path/query validation, bearer security, and shared safe errors while excluding internal metrics/training/source-intake surfaces |
...
| GitHub Actions `verify` run `29881792964` at `25db3e4` | Partially passed; superseding run required | Source verification, builds, all four rootless-container smokes, PostgreSQL restart evidence, dependency audit, and the Bluebeam browser workflow passed. The tablet job exposed an unrelated missing response synchronization in the existing project-controls look-ahead test; the test now waits for and validates the exact API response and passed three repeated local stress runs. The command-center commit requires a new hosted run before its hosted evidence can be claimed. |
