...
|---|---|---|
| `pnpm run verify` | Passed | Production/training boundary, secret scan, 84/25 requirement traceability with 240 evidence paths, generated OpenAPI drift, 12 Bicep templates, four container-definition targets, strict TypeScript, 102 unit/integration/security/acceptance tests, and compiled API/worker/browser process checks |
...
| `pnpm run database:verify` | Passed | Fourteen reversible migration pairs applied on disposable PostgreSQL 18.4; 61 controlled constraints and runtime/worker role boundaries; exact first-application-authority bootstrap/retry persistence; estimating, project-controls, WPS/WPQ/weld/NDE/PWHT/test, collaboration import/item/reconciliation, and fabrication assembly/traveler/hold-event typed hydration; immutable MTR/movement/report tables; rollback, atomic outbox, 2,000-record behavior, concurrent stale updates, and competing leases |
| `pnpm run test:browser` | Passed | Eleven tablet-Chromium tests covering project setup, the enterprise command center, estimating, project controls/procurement/scheduling, welding/NDE/PWHT/testing, governed Bluebeam collaboration, fabrication planning/spool control, the guided document-to-turnover chain, and partner boundaries; fail-closed states, scoped workflows, typed confirmation/current-version submission, 44-pixel actions, and zero serious/critical axe findings |
...
| `pnpm run openapi:verify` | Passed within `verify` | TypeScript-derived runtime schemas match 203 active `/v1` routes; generated OpenAPI 3.0.3 publishes current request bodies, path/query validation, bearer security, and shared safe errors while excluding internal metrics/training/source-intake surfaces |
