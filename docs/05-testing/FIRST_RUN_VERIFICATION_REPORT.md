...
The controlled MVP vertical slice and active expansion increments build and pass the available local automated gates. The evidence includes governed first-application-authority bootstrap and identity mapping; an isolated SHA-256-pinned persistent local-pilot runtime with expiring role-separated accounts; scoped authorization; guided project setup/readiness; authenticated files; document/material/PMI/inspection/NCR/punch/turnover/subcontractor workflows; advanced estimating and proposal handoff; integrated project controls, procurement, scheduling, and P6/Project draft import; exact WPS/PQR/WPQ and weld repair history; repair-cycle NDE; PWHT; completion-boundary testing; governed provider-neutral Bluebeam export preview/import/reconciliation/evidence review with outbound writes disabled; governed fabrication assemblies, revisioned spool records, operation travelers, hold/witness events, controlled work issue, completion, and as-built reconciliation; governed CNC/waterjet/profiling profiles, deterministic machine-neutral programs, validation, independent technical approval and release, exact-hash downloads, execution genealogy, and reconciliation with direct machine control prohibited; multidisciplinary engineering-register revisions with exact project/source relationships, deterministic validation and hash, independent approval, immutable supersession, audit, persistence, and responsive author/reviewer workflow; a unified permission-filtered command center with exact schedule lineage, owned/authorized work, separately gated audit activity, and module drill-through; 76 non-authoritative interactive review documents; all eleven MVP controlled outputs; interchange, background work, offline safety, recovery, provisional performance, browser accessibility, infrastructure compilation, and delivery controls.
...
|---|---|---|
| `pnpm run verify` | Passed | Production/training boundary, secret scan, 96/25 requirement traceability, generated OpenAPI drift, 12 Bicep templates, four container-definition targets, strict TypeScript, 112 unit/integration/security/acceptance tests, and compiled API/worker/browser process checks |
| `pnpm run pilot:status` | Passed | Isolated persistent PostgreSQL, API and worker are healthy; the UI is reachable at loopback; manifest-derived identities are expiring and development-only; health explicitly reports `productionReady: false` |
...
| `pnpm run database:verify` | Passed | Fourteen reversible migration pairs applied on disposable PostgreSQL 18.4; 61 controlled constraints and runtime/worker role boundaries; exact first-application-authority bootstrap/retry persistence; estimating, project-controls, WPS/WPQ/weld/NDE/PWHT/test, collaboration import/item/reconciliation, fabrication assembly/traveler/hold-event, CNC profile/program/execution, and engineering-register typed hydration; immutable MTR/movement/report tables; rollback, atomic outbox, 2,000-record behavior, concurrent stale updates, and competing leases |
| `pnpm run test:browser` | Passed | Thirteen tablet-Chromium tests covering project setup, the enterprise command center, estimating, project controls/procurement/scheduling, welding/NDE/PWHT/testing, governed Bluebeam collaboration, fabrication planning/spool control, CNC exact release/execution/reconciliation, multidisciplinary engineering registers, the guided document-to-turnover chain, and partner boundaries; fail-closed states, scoped workflows, typed confirmation/current-version submission, 44-pixel actions, and zero serious/critical axe findings |
...
| `pnpm run openapi:verify` | Passed within `verify` | TypeScript-derived runtime schemas match 219 active `/v1` routes and 173 request bodies; generated OpenAPI 3.0.3 publishes path/query validation, bearer security, and shared safe errors while excluding internal metrics/training/source-intake surfaces |
...
| GitHub Actions `verify` run `29890723406` at `7212976` | Passed | Clean hosted Linux verification covers 105 source tests, workspace builds, all four rootless-container builds/smokes, PostgreSQL 18 migrations/restart/concurrency including fabrication and CNC persistence, all twelve tablet/axe workflows including CNC exact release/execution/reconciliation, production dependency audit, generated evidence, and sealed retained artifacts. |
...
- AC-14: protected Bluebeam export/source identity, exact document/user/organization/status mapping, markup/comment/reply/page-region fidelity, idempotent atomic commit, changed-source and unsupported-content reconciliation, independent evidence review, and an explicit disabled outbound/write boundary.
- AC-15: permission-filtered on-demand module metrics, exact-current-revision schedule lineage/progress, explicitly owned or exactly authorized work, deterministic priority/due/overdue state, separately authorized safe audit activity, and accessible authoritative drill-through.
- AC-16: governed fabrication assemblies and revision ancestry, stable spool identity, exact drawing/material/BOM linkage, traveler operation and hold/witness control, authorized work issue/completion, immutable history, and independently reconciled as-built state.
- AC-17: independently approved effective machine profiles, exact source/fabrication/material lineage, deterministic machine-neutral program validation and hash, segregated technical approval and release, reauthorized exact-hash download, qualified execution genealogy, independent reconciliation, and an explicit no-direct-machine-control boundary.
- AC-18: stable revisioned engineering-register identity, exact project/structure/document/relationship scope, explicit deterministic validation findings and canonical hash, current-version submit/review, qualified independent authority and separation of duty, retained successor supersession history, scoped projection, audit, PostgreSQL hydration, and tablet accessibility.
