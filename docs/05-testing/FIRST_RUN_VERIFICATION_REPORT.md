...
|---|---|---|
| `pnpm run verify` | Passed | Production/training boundary, secret scan, 90/25 requirement traceability with 262 evidence paths, generated OpenAPI drift, 12 Bicep templates, four container-definition targets, strict TypeScript, 105 unit/integration/security/acceptance tests, and compiled API/worker/browser process checks |
...
| `pnpm run database:verify` | Passed | Fourteen reversible migration pairs applied on disposable PostgreSQL 18.4; 61 controlled constraints and runtime/worker role boundaries; exact first-application-authority bootstrap/retry persistence; estimating, project-controls, WPS/WPQ/weld/NDE/PWHT/test, collaboration import/item/reconciliation, fabrication assembly/traveler/hold-event, and CNC profile/program/execution typed hydration; immutable MTR/movement/report tables; rollback, atomic outbox, 2,000-record behavior, concurrent stale updates, and competing leases |
| `pnpm run test:browser` | Passed | Twelve tablet-Chromium tests covering project setup, the enterprise command center, estimating, project controls/procurement/scheduling, welding/NDE/PWHT/testing, governed Bluebeam collaboration, fabrication planning/spool control, CNC exact release/execution/reconciliation, the guided document-to-turnover chain, and partner boundaries; fail-closed states, scoped workflows, typed confirmation/current-version submission, 44-pixel actions, and zero serious/critical axe findings |
...
| `pnpm run openapi:verify` | Passed within `verify` | TypeScript-derived runtime schemas match 213 active `/v1` routes; generated OpenAPI 3.0.3 publishes current request bodies, path/query validation, bearer security, and shared safe errors while excluding internal metrics/training/source-intake surfaces |
