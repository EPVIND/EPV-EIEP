...
|---|---|---|
| `pnpm run verify` | Passed | Production/training boundary, secret scan, 96/25 requirement traceability with 278 evidence paths, generated OpenAPI drift, 12 Bicep templates, four container-definition targets, strict TypeScript, 107 unit/integration/security/acceptance tests, and compiled API/worker/browser process checks |
...
| `pnpm run openapi:verify` | Passed within `verify` | TypeScript-derived runtime schemas match 217 active `/v1` routes and 173 request bodies; generated OpenAPI 3.0.3 publishes path/query validation, bearer security, and shared safe errors while excluding internal metrics/training/source-intake surfaces |
